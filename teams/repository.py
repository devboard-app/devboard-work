...
async def get_teams_by_user(user_id: str, limit: int, offset: int) -> tuple[list[TeamMembership], int]:
    return await page(TeamMembership.objects.filter(user_id=user_id).select_related('team'), limit, offset)
...
def create_membership_sync(team: Team, user_id: str, role: str) -> TeamMembership:
    return TeamMembership.objects.create(team=team, user_id=user_id, role=role)
...
async def get_membership_by_team(team_id: str, limit: int, offset: int) -> tuple[list[TeamMembership], int]:
    return await page(TeamMembership.objects.filter(team_id=team_id), limit, offset)

async def delete_membership(membership: TeamMembership) -> None:
    await membership.adelete()

async def count_owners(team_id: str) -> int:
    return await TeamMembership.objects.filter(team_id=team_id, role=TeamMembership.Role.OWNER).acount()